    ...
    <div class="col-sm-6">
      <div class="card">
        <div class="card-body box-profile">
          ...
          <p class="text-md badge {{ $type == config('global.type.code.red') ? 'badge-danger' : 'badge-secondary' }}">{{ $status }}</p>
          <ul class="list-group list-group-unbordered mb-3">
            ...
            <li class="list-group-item">
              ...
              <p class="float-right mb-0">{{ $image }}</p>
            </li>
            @endif
            <li class="list-group-item">
              <b>Tgl. Diubah</b>
        ...
        <div class="card-footer">
          <div class="row justify-content-center">
            <div class="col-auto">
              @include('partials.button.back', array('class' => 'btn-sm', 'action' => route('transaction.expense.index')))
            </div>
            @if($is_update)
            <div class="col-auto">
              @include('partials.button.edit', array('class' => 'btn-sm', 'action' => route('transaction.expense.edit', ['id'
              => $id])))
            </div>
            <div class="col-auto">
              @include('partials.button.print', array('class' => 'btn-sm', 'action' => route('transaction.expense.index')))
            </div>
            @endif
          </div>
        </div>
