...
<div class="container-fluid">
  ...
  @endif
  @if ($is_pic)
  @include('contents.home.partials.pic')
  @endif
</div>
@endsection
@section('push-js')
...
<script type="text/javascript">
  $('select[name=year]').on('change', function() {
        ...
        var options = {
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: 'SERAPAN RAB ' + data.title + ' TA ' + data.year,
                color: "#212529",
                font: {
                  size: 20,
                },
      ...
      function drawColumnTable(data) {
        ...
        $.each(data, function(index, value) {
          if (index == 'year' || index == 'status' || index == 'title') {
            return;
          }
          ...
          if (index == 'series') {
            ...
            var col = '<td><i class="fas fa-square" style="color:#ffc107"></i> DANA RAB</td>';
          } else if (index == 'requested') {
            var col = '<td><i class="fas fa-square" style="color:#17a2b8"></i> DANA USULAN</td>';
          } else if (index == 'approved') {
            var col = '<td><i class="fas fa-square" style="color:#28a745"></i> DANA REALISASI</td>';
          } else if (index == 'process') {
            var col = '<td><i class="fas fa-square" style="color:#ff851b"></i> DANA ON PROCESS</td>';
          } else if (index == 'percent_request') {
            var col = '<td><i class="fas fa-square" style="color:#836200"></i> % PENGAJUAN</td>';
          } else if (index == 'percent_approve') {
            var col = '<td><i class="fas fa-square" style="color:#0b515c"></i> % REALISASI</td>';
          } else if (index == 'percent_progress') {
            var col = '<td><i class="fas fa-square" style="color:#145322"></i> % ON PROGRESS</td>';
          } else if (index == 'percent_process') {
            var col = '<td><i class="fas fa-square" style="color:#8d4100"></i> % ON PROCESS</td>';
          } else if (index == 'finished') {
            var col = '<td><i class="fas fa-square" style="color:#007bff"></i> STATUS SELESAI</td>';
          } else if (index == 'unfinished') {
            var col = '<td><i class="fas fa-square" style="color:#dc3545"></i> STATUS BELUM SELESAI</td>';
          }
          ...
          $.each(value, function(key, val) {
            if (index == 'series') {
              ...
              index == 'percent_process') {
              col += '<td class="text-center">' + val + '%</td>';
            } else if (index == 'finished' || index == 'unfinished') {
              col += '<td class="text-center">' + val + '</td>';
            } else {
              col += '<td class="text-center">' + formatCurrency(val) + '</td>';
            }
          })
          row.append(col);
          $('#note').find('table tbody').append(row);
